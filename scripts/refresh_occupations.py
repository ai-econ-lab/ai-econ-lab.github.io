...
def _row(name, name_sv, share, ads):
    """One occupation row, carrying its display label whenever the official title is too long.

    WHY THE SHORT NAME IS WRITTEN DOWN. It is computed at render time either way -- labels.py
    is the single source of truth and build.py calls it -- so this column is not an input.
    It is here so the shortening is VISIBLE in the data Magnus reads and in the diff: without
    it, a new SSYK title long enough to be shortened, or a change to the rule, moves what the
    site prints while data/ shows nothing at all. check_label_columns.py compares this column
    against labels.shorten() and fails if they disagree, so it cannot rot into a second answer.

    Emitted only when it differs from the official title, which is the useful signal: a
    name_short line in this file means "that one was too long".
    """
    row = {"name": name, "name_sv": name_sv}
    if shorten(name) != name:
        row["name_short"] = shorten(name)
    if shorten(name_sv, "sv") != name_sv:
        row["name_sv_short"] = shorten(name_sv, "sv")
    row.update({"share": share, "ads": ads, "is_se": False})
    return row
...
def main():
    ...
    doc = {
        ...
        "top": [_row(names.get(x["label"], x["label"]), x["label"], x["share"], x["ads"])
                for x in top],
        "zero": [_row(names.get(x["label"], x["label"]), x["label"], 0.0, x["ads"])
                 for x in zero],
