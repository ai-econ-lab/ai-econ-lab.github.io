...
# THE DEFINITION LABEL IS READ, NOT TYPED, for the same reason the counting unit above is
# emitted by the generator: this line said "frozen v1.1" while the composition underneath it
# had been rebuilt on v1.4, so the chart under-claimed its own currency for a day and would
# have gone on doing so at every future freeze. The candidate extract carries the definition
# that produced it; that stamp is the fact, and this is a label for it.
# VOCAB_FRAME_DIR names the frame that actually produced term_composition.csv. It defaults to
# the live extract. It exists because a rebuild can be in flight while the composition on disk
# still comes from the previous, archived frame: during the v1.5 rebuild the honest label was
# v1.4, and it came from data/candidates_v14_frame/. Pointing at the frame is still reading the
# provenance; typing "v1.4" into the output would not be.
_FRAME = Path(os.environ.get("VOCAB_FRAME_DIR",
                             str(MONITOR_ROOT / "data" / "candidates")))
_STAMP = json.loads((_FRAME / "_extract_definition.json").read_text(encoding="utf-8"))
if len(_STAMP.get("years_present", [])) < 22:
    raise SystemExit(
        f"refresh_vocabulary: the frame at {_FRAME.name} holds "
        f"{len(_STAMP.get('years_present', []))} of 22 periods, so it is mid-rebuild. "
        f"term_composition.csv would be built on a partial frame. Wait for the extract to "
        f"finish, then re-run.")
FRAME_VERSION = _STAMP["version"].split()[0]
...
lines = ["# Generated by scripts/refresh_vocabulary.py. Do not hand-edit.",
         ...
         f"  source: JobTech historical job ads (Arbetsförmedlingen), CC0, frozen {FRAME_VERSION} "
