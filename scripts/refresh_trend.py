...
def read_series():
    ...
    # The bare-AI band: advertisements that say AI and name no specific AI term. Emitted as a
    # third line because the RATIO of this band to the floor is a finding in its own right --
    # it ran at a fifth of the floor in 2016 and two and a half times it in 2025 -- and a
    # figure that lives only in a note is a figure nobody checks.
    band_raw = [100 * int(full[y]["band_bare_RAW"]) / int(full[y]["ads"]) for y in years]
...
def render(years, names, floor, band, provisional_from, defn, extra=None) -> str:
    extra = extra or {}
    ...
    lines = [
        ...
        "#",
        "# Three lines. values = advertisements that NAME a specific AI skill anywhere in the",
        "# text; floor_values = advertisements that ASK for one in the role's own requirements;",
        "# band_values = advertisements that say AI and name no specific AI term at all.",
        "# The band is the AI-talk line: it is not demand for an AI skill, and its ratio to the",
        "# floor is what the October brief reports.",
    ...
    lines += [
        ...
        f"  band_values: [{', '.join(f'{round(v, 3):g}' for v in band)}]",
    ]
